...
class UserLogin extends Model
{
    ...
    public static function createRememberCookie($userID)
    {
        ...
        if ($Db->query("SELECT COUNT(*) AS count FROM `user_cookies` WHERE `user_id` = {$userID}")->results()->count) {
            $hash = $Db->query("SELECT * FROM `user_cookies` WHERE `user_id` = {$userID}")->results()->hash;
        } else {
            $hash = Utility\Hash::generateUnique();
            if (!$Db->query("INSERT INTO `user_cookies` (`id`, `hash`, `user_id`) VALUES (NULL, '{$hash}', '{$userID}')")->results()) {
                return false;
            }
        }
    ...
    public static function loginWithCookie()
    {
        ...
        if ($Db->query("SELECT COUNT(*) AS count FROM `user_cookies` WHERE `hash` = {$hash}")->results()->count) {

            // Check if the user exists.
            $userID = $Db->query("SELECT * FROM `user_cookies` WHERE `hash` = {$hash}")->results(0)->user_id;
