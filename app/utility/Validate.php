...
class Validate
{
    ...
    public function check(array $inputs)
    {
        ...
        foreach ($inputs as $input => $rules) {
            if (isset($this->_source[$input])) {
                ...
                $this->_addError($input, Text::get("VALIDATE_MISSING_INPUT", ["%ITEM%" => mb_strtolower(Text::get("VALIDATE_" . strtoupper($input)))]));
            }
        }
    ...
    private function _validate($input, $value, array $rules)
    {
        foreach ($rules as $rule => $ruleValue) {
            if (($rule === "required" and $ruleValue === true) and empty($value)) {
                $this->_addError($input, Text::get("VALIDATE_REQUIRED_RULE", ["%ITEM%" => mb_strtolower(Text::get("VALIDATE_" . strtoupper($input)))]));
            } else if (!empty($value)) {
                $methodName = lcfirst(ucwords(strtolower(str_replace(["-", "_"], "", $rule)))) . "Rule";
                if (method_exists($this, $methodName)) {
                    $this->{$methodName}($input, $value, $ruleValue);
                } else {
                    $this->_addError($input, Text::get("VALIDATE_MISSING_METHOD", ["%ITEM%" => Text::get("VALIDATE_" . strtoupper($input))]));
                }
            }
        }
    }
        ...
        switch ($ruleValue) {
            ...
            case "email":
                if (!filter_var($value, FILTER_VALIDATE_EMAIL)) {
                    $data = [
                        "%ITEM%" => mb_strtolower(Text::get("VALIDATE_" . strtoupper($input))),
                        "%RULE_VALUE%" => Text::get("VALIDATE_" . strtoupper($ruleValue))
                    ];
    ...
    protected function matchesRule($input, $value, $ruleValue)
    {
        if ($value != $this->_source[$ruleValue]) {
            $data = [
                "%ITEM%" => mb_strtolower(Text::get("VALIDATE_MATCHES_" . strtoupper($input))),
                "%RULE_VALUE%" => Text::get("VALIDATE_MATCHES_" . strtoupper($ruleValue))
            ];
    ...
    protected function maxCharactersRule($input, $value, $ruleValue)
    {
        if (strlen($value) > $ruleValue) {
            $data = [
                "%ITEM%" => Text::get("VALIDATE_" . strtoupper($input)),
    ...
    protected function requiredRule($input, $value, $ruleValue)
    {
        if ($ruleValue === true and empty($value)) {
            $this->_addError($input, Text::get("VALIDATE_REQUIRED_RULE", ["%ITEM%" => Text::get("VALIDATE_" . strtoupper($input))]));
        }
    }
    ...
    {
        $check = $this->_Db->query("SELECT COUNT(*) AS count FROM `{$ruleValue}` WHERE `{$input}` = \"{$value}\"")->results();
        if ($check[0]->count) {
            if ($this->_recordID and $check->first()->id === $this->_recordID) {
                return;
            }
            $this->_addError($input, Text::get("VALIDATE_UNIQUE_RULE", ["%ITEM%" => Text::get("VALIDATE_" . strtoupper($input))]));
        }
    }
